...
export class SingaporeWeatherClient {
    ...
    async getCurrentWeather(latitude: number, longitude: number): Promise<WeatherSnapshot> {
        ...
        const [temperature, humidity, rainfall, windSpeed, windDirection, uvIndex] =
            await Promise.all([
        ...
        return {
            days: (item?.forecasts ?? [])
                .map((forecast) => ({
                    date:
                        forecast.date ??
                        dateFromTimestamp(forecast.timestamp) ??
                        forecast.day ??
                        '',
        ...
        try {
            const response = await fetch(url, {
                ...
                headers: {
                    ...
                    'User-Agent':
                        this.options.userAgent ?? 'weather-starter/0.1 (educational project)',
