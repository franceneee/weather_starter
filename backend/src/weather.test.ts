...
const stationReading = (stationId: string, value: number, lat: number, lon: number) => ({
  ...
  errorMsg: '',
  data: {
    stations: [{ id: stationId, name: 'Test Station', location: { latitude: lat, longitude: lon } }],
...
const twentyFourHrForecast = () => ({
  ...
  data: {
    records: [{
      ...
      periods: [
        { timePeriod: { text: 'Morning' }, regions: { central: { text: 'Partly Cloudy (Day)', code: 'PC' } } },
        { timePeriod: { text: 'Afternoon' }, regions: { central: { text: 'Thundery Showers', code: 'TL' } } },
      ],
    }],
  },
});
...
const fourDayForecast = () => ({
  items: [{
    ...
    forecasts: [
      { date: '2026-07-10', forecast: 'Partly Cloudy', temperature: { low: 25, high: 33 } },
...
describe('SingaporeWeatherClient', () => {
  ...
  describe('getCurrentWeather', () => {
    it('assembles a full snapshot from all endpoints', async () => {
      ...
      expect(snapshot.forecast_periods[0]).toEqual({ label: 'Morning', forecast: 'Partly Cloudy (Day)' });
      expect(snapshot.daily_forecast).toHaveLength(2);
      expect(snapshot.daily_forecast[0]).toMatchObject({ date: '2026-07-10', forecast: 'Partly Cloudy', temperature_low_c: 25, temperature_high_c: 33 });
    });
  ...
  describe('fetchNearestReading', () => {
    it('picks the nearest station by coordinates', async () => {
      vi.stubGlobal('fetch', makeFetch({
        'air-temperature': {
          ...
          data: {
            stations: [
              { id: 'NEAR', name: 'Near', location: { latitude: LAT + 0.001, longitude: LON + 0.001 } },
              { id: 'FAR', name: 'Far', location: { latitude: LAT + 0.5, longitude: LON + 0.5 } },
            ],
            readings: [{ timestamp: '2026-07-10T10:00:00+08:00', data: [{ stationId: 'NEAR', value: 30 }, { stationId: 'FAR', value: 99 }] }],
          },
        },
      }));
    ...
    it('returns null when no readings available', async () => {
      vi.stubGlobal('fetch', makeFetch({
        'air-temperature': { code: 0, errorMsg: '', data: { stations: [], readings: [] } },
      }));
    ...
    it('throws WeatherProviderError on API error code', async () => {
      ...
      const client = new SingaporeWeatherClient();
      await expect(client.fetchNearestReading('rainfall', LAT, LON)).rejects.toBeInstanceOf(WeatherProviderError);
    });
  });
  ...
  describe('fetchAirQuality', () => {
    const makePsi = (region: string, psi: number) => ({
      code: 0, errorMsg: '',
      data: {
        regionMetadata: [{ name: region, labelLocation: { latitude: LAT, longitude: LON } }],
        items: [{
          timestamp: '2026-07-10T10:00:00+08:00',
      ...
      vi.stubGlobal('fetch', makeFetch({
        '/psi': {
          ...
          data: {
            ...
            items: [{ updatedTimestamp: '2026-07-10T10:00:00+08:00', readings: { psi_twenty_four_hourly: { central: 50, east: 70 } } }],
          },
        },
        '/pm25': {
          code: 0, errorMsg: '',
          data: {
            regionMetadata: regions,
            items: [{ updatedTimestamp: '2026-07-10T10:00:00+08:00', readings: { pm25_one_hourly: { central: 10, east: 20 } } }],
          },
        },
      }));
    ...
    it('throws WeatherProviderError when psi returns error code', async () => {
      ...
      const client = new SingaporeWeatherClient();
      await expect(client.fetchAirQuality(LAT, LON)).rejects.toBeInstanceOf(WeatherProviderError);
    });
  ...
  describe('fetchTwentyFourHourForecast', () => {
    it('extracts temp range and region-matched periods', async () => {
      ...
      expect(result.periods[0]).toEqual({ label: 'Morning', forecast: 'Partly Cloudy (Day)' });
    ...
    it('throws WeatherProviderError on non-zero error code', async () => {
      vi.stubGlobal('fetch', makeFetch({
        'twenty-four-hr-forecast': { code: 1, errorMsg: 'Upstream error', data: undefined },
      }));

      const client = new SingaporeWeatherClient();
      await expect(client.fetchTwentyFourHourForecast(LAT, LON)).rejects.toBeInstanceOf(WeatherProviderError);
    });
      ...
      vi.stubGlobal('fetch', makeFetch({
        '4-day-weather-forecast': {
          items: [{
            update_timestamp: '2026-07-10T08:00:00+08:00',
            forecasts: [
              { date: '2026-07-10', forecast: 'Fair', temperature: { low: 25, high: 32 } },
              { date: '', forecast: 'Cloudy', temperature: { low: 24, high: 31 } },      // empty date → filtered
              { date: '2026-07-12', forecast: '', temperature: { low: 24, high: 31 } },  // empty forecast → filtered
            ],
          }],
        },
      }));
    ...
    it('uses timestamp as date fallback when date field is absent', async () => {
      vi.stubGlobal('fetch', makeFetch({
        '4-day-weather-forecast': {
          items: [{
            update_timestamp: '2026-07-10T08:00:00+08:00',
            forecasts: [{ timestamp: '2026-07-10T00:00:00+08:00', forecast: 'Windy', temperature: { low: 24, high: 30 } }],
          }],
        },
      }));
          ...
          data: {
            records: [{
              ...
              forecasts: [{
                ...
                forecast: { summary: 'Partly cloudy', text: 'Partly Cloudy (Day)', code: 'PC' },
